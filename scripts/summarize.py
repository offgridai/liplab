    ...
    if summary.get("conditioned_landmark_audit_available", False):
        ...
        for landmark_type in ("mbp", "fv", "w", "chjjsh", "round", "comma_lull"):
            ...
            if target_count <= 0:
                continue
            chunks.append(
                f"{landmark_type}:targets={target_count},obs={int(summary.get(f'conditioned_landmark_{landmark_type}_observation_count', 0))},"
                f"recall={float(summary.get(f'conditioned_landmark_{landmark_type}_recall', 0.0)):.3f},"
                f"precision={float(summary.get(f'conditioned_landmark_{landmark_type}_precision', 0.0)):.3f},"
                f"hit={float(summary.get(f'conditioned_landmark_{landmark_type}_target_window_hit_rate', 0.0)):.3f},"
                f"center_ms={float(summary.get(f'conditioned_landmark_{landmark_type}_mean_abs_center_error_ms', 0.0)):.1f},"
