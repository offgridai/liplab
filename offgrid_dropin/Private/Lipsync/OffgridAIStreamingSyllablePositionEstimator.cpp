...
static TArray<FOffgridAIStreamingSyllablePositionEstimate> EstimateCausalAnchors(
    const FOffgridAITextVisemePlan& Plan,
    const TArray<FOffgridAIAudioLandmarkObservation>& EvidenceCandidates)
{
    const TArray<FSyllableTarget> Targets = BuildTargets(Plan);
    ...
    int32 LastAssignedTarget = INDEX_NONE;
    for (int32 PrefixCount = 1; PrefixCount <= Pulses.Num(); ++PrefixCount)
    {
        ...
        auto FindLatestMatch = [&](int32 TerminalRow, int32& OutTarget, int32& OutPulse, float& OutScore)
        {
            int32 TraceRow = TerminalRow;
            int32 TraceColumn = PrefixCount;
            OutTarget = INDEX_NONE;
            OutPulse = INDEX_NONE;
            OutScore = 0.0f;
            while (TraceRow > 0 || TraceColumn > 0)
            {
                ...
                if (Step == 3)
                {
                    if (OutPulse == INDEX_NONE)
                    {
                        OutTarget = TraceRow - 1;
                        OutPulse = TraceColumn - 1;
                        OutScore = Scores[Cell(TraceRow, TraceColumn)]
        ...
        int32 MatchedTarget = INDEX_NONE;
        int32 MatchedPulse = INDEX_NONE;
        float MatchedScore = 0.0f;
        FindLatestMatch(BestTerminalRow, MatchedTarget, MatchedPulse, MatchedScore);
        if (MatchedPulse == INDEX_NONE) continue;
        const int32 RollbackDepth = LastAssignedTarget - MatchedTarget;
        const bool bRevisableRollback = RollbackDepth >= 1 && RollbackDepth <= 2
            && Out.Num() > 0
            && CurrentDecisionSec - Out.Last().DecisionSec <= 0.350f;
        if (MatchedTarget < LastAssignedTarget && !bRevisableRollback) continue;
        if (MatchedTarget == LastAssignedTarget) continue;
        if (bRevisableRollback)
        {
            while (Out.Num() > 0 && Out.Last().SyllableIndex > MatchedTarget)
                Out.RemoveAt(Out.Num() - 1);
        }
        LastAssignedTarget = MatchedTarget;
        const auto& Target = Targets[MatchedTarget];
        FOffgridAIStreamingSyllablePositionEstimate Estimate;
        Estimate.SyllableIndex = MatchedTarget;
        Estimate.NucleusPhoneIndex = Target.NucleusPhoneIndex;
        Estimate.WordIndex = Target.WordIndex;
        Estimate.SpeechRegionIndex = Target.SpeechRegionIndex;
        Estimate.AudioCenterSec = Pulses[MatchedPulse].CenterSec;
        Estimate.DecisionSec = CurrentDecisionSec;
        Estimate.MatchScore = MatchedScore;
        Estimate.Confidence = FMath::Clamp((MatchedScore - 0.45f) / 1.20f, 0.0f, 1.0f);
        Out.Add(Estimate);
    }
    return Out;
}
...
{
    TArray<FSyllableTarget> Targets;
    for (const FSyllableTarget& Target : BuildTargets(Plan))
    {
        if (TextSpeechRegionIndex == INDEX_NONE
            || Target.SpeechRegionIndex == TextSpeechRegionIndex)
            Targets.Add(Target);
    }
    TArray<FOffgridAIAudioLandmarkObservation> Pulses;
    TArray<FOffgridAIAudioLandmarkObservation> PhoneCandidates;
    for (const FOffgridAIAudioLandmarkObservation& Candidate : EvidenceCandidates)
    {
        if (Candidate.CenterSec < AudioStartSec || Candidate.CenterSec > AudioEndSec)
            continue;
        if (Candidate.Type == EOffgridAIAudioLandmarkType::SyllabicPulse)
            Pulses.Add(Candidate);
        else if (IsPhoneFamily(Candidate.Type)
            && Candidate.Type != EOffgridAIAudioLandmarkType::OpenVowel
            && Candidate.Type != EOffgridAIAudioLandmarkType::FrontVowel)
            PhoneCandidates.Add(Candidate);
    }
    if (Targets.Num() <= 0 || Pulses.Num() < Targets.Num()) return {};

    TArray<float> ExpectedProgress;
    const float FirstGap = Targets.Num() > 1
        ? FMath::Max(Targets[1].PriorCenterSec - Targets[0].PriorCenterSec, 0.025f)
        : 0.160f;
    const float LastGap = Targets.Num() > 1
        ? FMath::Max(Targets.Last().PriorCenterSec - Targets[Targets.Num() - 2].PriorCenterSec, 0.025f)
        : 0.160f;
    const float PriorLow = Targets[0].PriorCenterSec - FirstGap * 0.5f;
    const float PriorSpan = FMath::Max(
        Targets.Last().PriorCenterSec + LastGap * 0.5f - PriorLow,
        0.001f);
    for (const FSyllableTarget& Target : Targets)
        ExpectedProgress.Add((Target.PriorCenterSec - PriorLow) / PriorSpan);

    const float AudioSpan = FMath::Max(AudioEndSec - AudioStartSec, 0.001f);
    const int32 Rows = Targets.Num();
    const int32 Columns = Pulses.Num();
    const float Infinity = 1.0e9f;
    std::vector<float> Costs(static_cast<size_t>(Rows * Columns), Infinity);
    std::vector<int32> BackColumns(static_cast<size_t>(Rows * Columns), INDEX_NONE);
    auto Cell = [Columns](int32 Row, int32 Column)
    {
        return static_cast<size_t>(Row * Columns + Column);
    };
    auto LocalCost = [&](int32 Row, int32 Column)
    {
        const float AudioProgress = (Pulses[Column].CenterSec - AudioStartSec) / AudioSpan;
        float LandmarkSupport = 0.0f;
        for (const FFamilyOffset& Expected : Targets[Row].FamilyOffsets)
        {
            float Best = 0.0f;
            const float ExpectedCenter = Pulses[Column].CenterSec + Expected.OffsetSec;
            for (const FOffgridAIAudioLandmarkObservation& Candidate : PhoneCandidates)
            {
                if (Candidate.Type != Expected.Type) continue;
                const float Proximity = FMath::Clamp(
                    1.0f - FMath::Abs(Candidate.CenterSec - ExpectedCenter) / 0.120f,
            ...
            LandmarkSupport += Best;
        }
        LandmarkSupport /= FMath::Max(Targets[Row].FamilyOffsets.Num(), 1);
        return 0.25f * FMath::Abs(ExpectedProgress[Row] - AudioProgress)
            - 0.40f * Pulses[Column].Score
            - 0.20f * LandmarkSupport;
    };

    for (int32 Column = 0; Column < Columns; ++Column)
        Costs[Cell(0, Column)] = LocalCost(0, Column);
    for (int32 Row = 1; Row < Rows; ++Row)
    {
        const float ExpectedGap = ExpectedProgress[Row] - ExpectedProgress[Row - 1];
        for (int32 Column = Row; Column < Columns; ++Column)
        {
            const float Local = LocalCost(Row, Column);
            const float AudioProgress = (Pulses[Column].CenterSec - AudioStartSec) / AudioSpan;
            for (int32 Previous = Row - 1; Previous < Column; ++Previous)
            {
                const float PreviousProgress =
                    (Pulses[Previous].CenterSec - AudioStartSec) / AudioSpan;
                const float Cost = Costs[Cell(Row - 1, Previous)]
                    + Local
                    + 2.0f * FMath::Abs(ExpectedGap - (AudioProgress - PreviousProgress));
                if (Cost < Costs[Cell(Row, Column)])
                {
                    Costs[Cell(Row, Column)] = Cost;
                    BackColumns[Cell(Row, Column)] = Previous;
                }
            }
        }
    }

    int32 FinishColumn = Rows - 1;
    for (int32 Column = Rows; Column < Columns; ++Column)
    {
        if (Costs[Cell(Rows - 1, Column)] < Costs[Cell(Rows - 1, FinishColumn)])
            FinishColumn = Column;
    }
    TArray<int32> SelectedColumns;
    SelectedColumns.SetNum(Rows);
    for (int32 Row = Rows - 1; Row >= 0; --Row)
    {
        SelectedColumns[Row] = FinishColumn;
        if (Row > 0)
        {
            FinishColumn = BackColumns[Cell(Row, FinishColumn)];
            if (FinishColumn == INDEX_NONE) return {};
        }
    }
    ...
    {
        const auto& Target = Targets[Row];
        const auto& Pulse = Pulses[SelectedColumns[Row]];
        FOffgridAIStreamingSyllablePositionEstimate Estimate;
        Estimate.SyllableIndex = Target.SyllableIndex;
        Estimate.NucleusPhoneIndex = Target.NucleusPhoneIndex;
        Estimate.WordIndex = Target.WordIndex;
        ...
        Estimate.AudioCenterSec = Pulse.CenterSec;
        Estimate.DecisionSec = Pulse.DecisionSec;
        Estimate.MatchScore = Pulse.Score;
        Estimate.Confidence = FMath::Clamp(Pulse.Score, 0.0f, 1.0f);
        Out.Add(Estimate);
    }
    return Out;
}
...
TArray<FOffgridAIStreamingDensePositionEstimate>
    ...
    float MinMatchScore)
{
    ...
    const TArray<FOffgridAIStreamingSyllablePositionEstimate> Anchors =
        EstimateHistoricalAnchors(
            Plan,
            EvidenceCandidates,
            ObservedSpeechRegions,
            RequiredStableUpdates,
            MinMatchScore);
    TArray<FOffgridAIStreamingDensePositionEstimate> Out;
    if (Targets.Num() <= 0 || Anchors.Num() <= 0) return Out;

    int32 LastOutputSyllable = 0;
    for (const auto& Frame : Frames)
    {
        if (!Frame.bInSpeechAfterFrame) continue;
        const float PlaybackDeadlineSec = Frame.AudioBufferCenterSec + 0.350f;
        const FOffgridAIStreamingSyllablePositionEstimate* Anchor = nullptr;
        for (const auto& Candidate : Anchors)
        {
            if (Candidate.DecisionSec > PlaybackDeadlineSec) continue;
            if (Anchor == nullptr || Candidate.DecisionSec > Anchor->DecisionSec)
                Anchor = &Candidate;
        }
        if (Anchor == nullptr || !Targets.IsValidIndex(Anchor->SyllableIndex)) continue;

        const float ProjectedPriorSec = Targets[Anchor->SyllableIndex].PriorCenterSec
            + (Frame.AudioBufferCenterSec - Anchor->AudioCenterSec);
        int32 SelectedSyllable = Anchor->SyllableIndex;
        float BestDistance = TNumericLimits<float>::Max();
        for (int32 Index = Anchor->SyllableIndex; Index < Targets.Num(); ++Index)
        {
            const float Distance = FMath::Abs(Targets[Index].PriorCenterSec - ProjectedPriorSec);
            if (Distance < BestDistance)
            {
                BestDistance = Distance;
                SelectedSyllable = Index;
            }
            else if (Targets[Index].PriorCenterSec > ProjectedPriorSec) break;
        }
        LastOutputSyllable = FMath::Max(LastOutputSyllable, SelectedSyllable);
        if (!Targets.IsValidIndex(LastOutputSyllable)) continue;
        const auto& Target = Targets[LastOutputSyllable];
        const float Duration = FMath::Max(Target.PriorEndSec - Target.PriorStartSec, 0.060f);
        FOffgridAIStreamingDensePositionEstimate Estimate;
        Estimate.AudioCenterSec = Frame.AudioBufferCenterSec;
        Estimate.SyllableIndex = LastOutputSyllable;
        Estimate.NucleusPhoneIndex = Target.NucleusPhoneIndex;
        Estimate.WordIndex = Target.WordIndex;
        Estimate.SyllablePhase = FMath::Clamp(
            0.5f + (ProjectedPriorSec - Target.PriorCenterSec) / Duration,
            0.0f,
            1.0f);
        Estimate.LocalRate = 1.0f;
        Estimate.Confidence = Anchor->Confidence;
        Estimate.RunnerUpMargin = Anchor->Confidence;
        Estimate.LastAnchorAgeSec = FMath::Max(
            Frame.AudioBufferCenterSec - Anchor->AudioCenterSec,
            0.0f);
        Out.Add(Estimate);
    }
...
{
    const TArray<FSyllableTarget> Targets = BuildTargets(Plan);
    TArray<FOffgridAIStreamingSyllablePositionEstimate> Anchors = EstimateCausalAnchors(
        Plan,
        EvidenceCandidates);
    TArray<FOffgridAIStreamingDensePositionEstimate> Out;
    if (Targets.Num() <= 0 || Anchors.Num() <= 0) return Out;
    ...
    {
        if (!Frame.bInSpeechAfterFrame) continue;
        const FOffgridAIStreamingSyllablePositionEstimate* Previous = nullptr;
        const FOffgridAIStreamingSyllablePositionEstimate* Next = nullptr;
        for (const auto& Anchor : Anchors)
        {
            // The anchor must be decidable before this audio reaches playback;
            // the default preroll exposes up to 350 ms of future evidence.
            if (Anchor.DecisionSec > Frame.AudioBufferCenterSec + 0.350f) continue;
            if (Anchor.AudioCenterSec <= Frame.AudioBufferCenterSec)
            {
                if (Previous == nullptr || Anchor.AudioCenterSec > Previous->AudioCenterSec)
                {
                    Previous = &Anchor;
                }
            }
            else if (Next == nullptr || Anchor.AudioCenterSec < Next->AudioCenterSec)
                Next = &Anchor;
        }
        const FOffgridAIStreamingSyllablePositionEstimate* Best = Previous != nullptr
            ? Previous
            : (Next != nullptr ? Next : &Anchors[0]);
        int32 SelectedSyllable = Best->SyllableIndex;
        if (Previous != nullptr && Next != nullptr
            && Next->SyllableIndex > Previous->SyllableIndex
            && Next->AudioCenterSec - Previous->AudioCenterSec >= 0.060f)
        {
            const float AudioProgress = FMath::Clamp(
                (Frame.AudioBufferCenterSec - Previous->AudioCenterSec)
                    / (Next->AudioCenterSec - Previous->AudioCenterSec),
                0.0f,
                1.0f);
            const float PriorPositionSec = FMath::Lerp(
                Targets[Previous->SyllableIndex].PriorCenterSec,
                Targets[Next->SyllableIndex].PriorCenterSec,
                AudioProgress);
            float BestPriorDistance = TNumericLimits<float>::Max();
            for (int32 Index = Previous->SyllableIndex; Index <= Next->SyllableIndex; ++Index)
            {
                const float Distance = FMath::Abs(Targets[Index].PriorCenterSec - PriorPositionSec);
                if (Distance < BestPriorDistance)
                {
                    BestPriorDistance = Distance;
                    SelectedSyllable = Index;
                }
            }
        }
        ...
        const auto& Target = Targets[LastOutputSyllable];
        const float Duration = FMath::Max(Target.PriorEndSec - Target.PriorStartSec, 0.060f);
        const float BestDistance = FMath::Abs(Best->AudioCenterSec - Frame.AudioBufferCenterSec);
        const float Phase = FMath::Clamp(
            0.5f + (Frame.AudioBufferCenterSec - Best->AudioCenterSec) / Duration,
            0.0f,
            1.0f);
        FOffgridAIStreamingDensePositionEstimate EstimateRow;
        EstimateRow.AudioCenterSec = Frame.AudioBufferCenterSec;
        EstimateRow.SyllableIndex = LastOutputSyllable;
        EstimateRow.NucleusPhoneIndex = Target.NucleusPhoneIndex;
        EstimateRow.WordIndex = Target.WordIndex;
        EstimateRow.SyllablePhase = Phase;
        EstimateRow.LocalRate = 1.0f;
        EstimateRow.RunnerUpMargin = Best->Confidence;
        EstimateRow.Confidence = FMath::Clamp(
            Best->Confidence * (1.0f - BestDistance / 0.450f),
            0.0f,
            1.0f);
        EstimateRow.LastAnchorAgeSec = FMath::Max(
            Frame.AudioBufferCenterSec - Best->AudioCenterSec,
            0.0f);
        Out.Add(EstimateRow);
    }
    return Out;
