...
void FOffgridAIStreamingSpeechDetector::ProcessAnalysisFrame(float FrameStartSeconds, float FrameEndSeconds, float RMS)
{
    ...
    // M09: keep speech/pause detection responsive to brief TTS hesitations,
    // but close speech only on sustained low energy rather than arbitrary
    // intra-speech valleys.
