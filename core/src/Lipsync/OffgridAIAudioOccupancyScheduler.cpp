    ...
    bool& bOutHasTrack)
{
    ...
    for (int32 EventI = 0; EventI < TextPlan.Events.Num(); ++EventI)
    {
        ...
        const float TargetActiveElapsedSec = TextIslandInfo
            ? TextIslandInfo->AudioActiveStartSec + IslandLocalNorm * FMath::Max(TextIslandInfo->AudioActiveSpanSec, IslandTextSpanSec)
            : FMath::Max(0.0f, TextCenterSec - FirstTextCenterSec);
        const float TargetIslandLocalActiveSec = TextIslandInfo
            ? IslandLocalNorm * FMath::Max(TextIslandInfo->AudioActiveSpanSec, IslandTextSpanSec)
        ...
        Added.CommitReason = bMappedToObservedSpeech ? FName(TEXT("M09_audio_occupancy")) : FName(TEXT("M09_live_tail_drain_after_audio"));
        Added.EffectiveCommitHorizonSeconds = PrerollSec;
        Added.EffectiveSegmentScale = 1.0f;
        ...
        Added.PlannerDurationBucket = FName(TEXT("M09_audio_occupancy"));
        Added.PlannerDurationScaleApplied = 1.0f;
