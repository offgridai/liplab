...
struct FOffgridAITextVisemePlan
{
    ...
    // Planner metadata for every tokenized dialogue word, including words
    // that later produce no visible viseme event. The active runtime consumes
    // the resulting monotonic viseme timeline; word metadata remains useful
    // for diagnostics and future text-duration modeling.
    TArray<int32> WordSentenceIslandIndices;
    TArray<int32> WordPhraseIndices;
    TArray<int32> WordSyllableCounts;

    // Punctuation immediately following each tokenized word. Punctuation may
    // influence the planned text/prosody prior, but the M09 runtime does not
    // use punctuation as a streaming gate. Zero means no punctuation boundary
    // after the word.
